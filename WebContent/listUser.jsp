...
.page span span {
	...
	<a href="UserListServlet" class="btn disabled">用户管理</a>
	<a href="AdminSongListServlet">歌曲管理</a>
	...
	<table style="width:800px; margin:20px auto" class="table table-striped table-hover table-condensed">
		...
		<c:forEach items="${users }" var="user">
			<tr>
				...
				<td>${user.gender }</td>
				<td>${user.birthday }</td>
				<td><a href="javascript:if(confirm('确认删除用户    ${user.nickname }  ？')){location='DeleteUserServlet?account=${user.account }'}">删除</a></td>
			</tr>
		</c:forEach>	
	</table>
